 ...
 * AddDeliveryStopState is the class representing the state entered to add a delivery stop when wanting
 * to add a new request to the tour. It is characterized by :
 * <ul>
 * <li>A controller that contains some attributes and methods that
 * AddDeliveryStopState class needs to access.</li>
 * </ul>
 * 
 * @see Controller
 * @see State
 ...
 * @author H4122
 */
public class AddDeliveryStopState implements State {

	protected Controller controller;

	/**
	 * Constructor of AddDeliveryStopState. When the AddDeliveryStopState is created,its
	 * controller is initialized with a controller passed as a parameter.
	 * 
	 * @param newController The controller of AddDeliveryStopState that contains some
	 *                      attributes and methods that the class needs to access.
	 * 
	 * @see AddDeliveryStopState#controller
	 */
	public AddDeliveryStopState(Controller newController) {
	...
	/**
	 * Changes state to AddDeliveryPredecessorState
	 */
	@Override
	public void clicIntersection(Stop stopCreated) {
		try {
			Stop stop = stopCreated;
			stop.setOriginatedSegments(stopCreated.getOriginatedSegments());
			controller.getMap().getTour().setDeliveryStop(stop);
			controller.setCurrentState(controller.getAddDeliveryPredecessorState());
